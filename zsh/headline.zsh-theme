...
HEADLINE_NOBRANCH_PREFIX='(~'
if [ $IS_SSH = 0 ]; then HEADLINE_USER_PREFIX=' '; fi
...
headline_precmd() {
  ...
  if (( ${#branch_str} )); then
    ...
    _headline_part BRANCH "$HEADLINE_BRANCH_PREFIX%$len<$HEADLINE_TRUNC_PREFIX<$branch_str%<<$HEADLINE_BRANCH_END" right
  else
    _headline_part BRANCH "$HEADLINE_NOBRANCH_PREFIX$HEADLINE_BRANCH_END" right
  fi
